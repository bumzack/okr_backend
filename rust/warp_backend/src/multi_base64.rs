...
pub fn article_routes_multi_base64(
    pool: Pool,
) -> impl Filter<Extract=(impl Reply, ), Error=Rejection> + Clone {
    let server1 = warp::path!("multithreaded" / "api" / "articles" / "base64");
    let find_all_multi = server1
        .and(with_db(pool.clone()))
        .and(warp::get())
        .and_then(|pool: Pool| {
            info!("GET /multithreaded/api/articles/base64/");
            find_all_multi(pool)
        });

    let server1 = warp::path!("multithreaded" / "api" / "articles" / "base64" / u32 / u32);
    let find_paginated_multi = server1
        .and(warp::get())
        .and(with_db(pool.clone()))
        .and_then(|page_number: u32, page_size: u32, pool: Pool| {
            info!("GET /multithreaded/api/articles/base64/{page_number}/{page_size}");
