...
IMPORT_URL=(    "http://okr-backend:2323/api/v1/articles/import"   )
SYSINFO_URL=(   "http://okr-backend:2323/api/v1/sysinfo"           )
...
do
    ...
    do
        DURATION=$(curl -w "@curl-format.txt" -s -o /dev/null -X POST  ${URL}  -d '{ "returnItems": false  }'  -H "Content-Type: application/json" -H "Cache-Control: no-cache, no-store" )
